...
import app.blownchart.ui.preferences.destinations.DockPreferencesPreview
import app.blownchart.ui.preferences.navigation.DockSearchProvider
...
fun DockSearchPreference(
    ...
    Crossfade(isHotseatEnabled.state.value, label = "transition", modifier = modifier) { hotseatEnabled ->
        val isBlownChartHotseat = hotseatModeAdapter.state.value == BlownChartHotseat
        if (hotseatEnabled) {
            Column {
                PreferenceGroup {
                ...
                ExpandAndShrink(visible = isBlownChartHotseat) {
                    Column {
