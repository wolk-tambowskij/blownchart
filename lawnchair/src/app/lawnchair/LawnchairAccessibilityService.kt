...
import app.lawnchair.util.hasUsageStatsAccess
...
class LawnchairAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        when (event?.eventType) {
            ...
            AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED -> {
                if (event.packageName?.toString() != lawnchairApp.systemRecentsComponentName?.packageName) return
    ...
    private fun isLabelVisibleInRecents(label: String): Boolean {
        val recentsPackage = lawnchairApp.systemRecentsComponentName?.packageName ?: return false
        val windowList = try {
            windows
        } catch (e: SecurityException) {
            return false
    ...
    private fun findRecentForegroundPackages(limit: Int): List<String> {
        ...
        val recentsProviderPackage = lawnchairApp.systemRecentsComponentName?.packageName
