...
| 15 | `fix/loader-model-thread-priority` | Home-screen/app-drawer/settings loading performance: reference-counted thread-priority elevation (was racing raw `setThreadPriority()` calls) + bulk icon loading in the app list instead of one query per app | M | Partial credit to Lawnchair 16's own upstream fix for the thread-priority half (Bug: 396250724) - this PR's bulk-icon-loading half is separate, unsolicited | ready for review (`docs/pr/fix-loader-startup-and-list-performance.md` was written 2026-08-01 but never got a table row - pure oversight, added 2026-08-10 while auditing for untracked work. Not yet re-verified against a clean-room branch - **no `origin/fix/loader-model-thread-priority` branch exists yet**, this row tracks the fork-side fix and doc only; branch creation is the next step before this can move to "ready for review" in the branch sense) |
