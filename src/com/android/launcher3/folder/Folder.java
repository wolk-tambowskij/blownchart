...
public class Folder extends AbstractFloatingView implements ClipPathView, DragSource,
    ...
    /**
     * A downstream fork may show this Folder for an app-drawer folder that isn't tracked in the
     * real LauncherModel at all - see isInAppDrawer()'s own doc. Such a FolderInfo's id can come
     * from an entirely separate id sequence than the model's own, and can coincidentally collide
     * with a real, unrelated item's id in the model (e.g. a hotseat app). Passing one of these
     * FolderInfo objects to ModelWriter validates it against whatever real item happens to share
     * that id and throws when it doesn't match. Use this in place of
     * mLauncherDelegate.getModelWriter() for the (null-safe) FolderInfo.setTitle()/setOption()
    ...
    private void createNestedFolder(DragObject d, View targetView) {
        ...
        writer.addOrMoveItemInDatabase(targetInfo, newFolderInfo.id, 0, targetInfo.cellX, targetInfo.cellY);
        newFolderInfo.add(draggedInfo, false);
        writer.addOrMoveItemInDatabase(draggedInfo, newFolderInfo.id, 0, draggedInfo.cellX, draggedInfo.cellY);

        // Add before removing (same reasoning as LauncherDelegate#replaceNestedFolderWithFinalItem):
        // keeps this folder's own item count from dipping to its post-merge value early, which
        // could otherwise wrongly trigger its own collapse-to-last-item check partway through.
